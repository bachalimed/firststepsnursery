...
const baseQuery = fetchBaseQuery({
  ...
  prepareHeaders: (headers, { getState }) => {
    //destructured api.getState , prepareheaders is function that will be applied to every request we send maybe add the selected year here
    const token = getState().auth.token; //getting the token from the state getStateAPI which is already defined

    if (token) {
      headers.set("Authorization", `Bearer ${token}`); //setting the token in the headers preceded with Bearer //////was authorization
    }
    return headers;
  },
});
...
export const apiSlice = createApi({
  ...
  tagTypes: [
    ...
    "employeeDocument",
    "service",
  ], //will be used for cached data so when we invalidate data, it will updated
