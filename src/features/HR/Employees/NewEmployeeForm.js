...
//console.log(` ${userFirstName}, ${validUserLastName}, ${validUsername}, ${validPassword}, ${validUserDob},${userAllowedActions}    ${ validStreet},  ${validPrimaryPhone}, ${validEmail}, ${userRoles.length}, ${isParent}, ${employeeId}, ${userIsActive}` )
    const onSaveUserClicked = async (e) => {
        ...
        if (canSave) {//if cansave is true
            ...
            await addNewEmployee({ username, password,  userFullName, isParent, employeeId, userDob, userSex,  userIsActive, userRoles, userAllowedActions, userAddress, userContact })//we call the add new user mutation and set the arguments to be saved
...
//the error messages to be displayed in every case according to the class we put in like 'form input incomplete... which will underline and highlight the field in that cass
    ...
    const content = (
        <>
        <Employees/>
            <p className={errClass}>{employeeError?.data?.message}</p>  {/*will display if there is an error message, some of the error messagees are defined in the back end responses*/}
