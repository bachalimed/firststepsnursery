...
const NewNotificationForm = () => {
  ...
  ] = useAddNewNotificationMutation(); //it will not execute the mutation nownow but when called
  ...
  const [userRoles, setUserRoles] = useState(["Notification"]); //the roles array is defaulted to notification
  const [userAllowedActions, setUserAllowedActions] = useState([]); //the roles array is defaulted to notification
  ...
  const [notificationAssessment, setNotificationAssessment] = useState([]);
  const [date, setDate] = useState("");
  const [assessor, setAssessor] = useState("");
  const [assessmentComment, setAssessorComment] = useState("");
  const [assessmentScore, setAssessmentScore] = useState("");
  const [notificationWorkHistory, setNotificationWorkHistory] = useState("");
  ...
  const [notificationCurrentEmployment, setNotificationCurrentEmployment] =
    useState("");
  ...
  useEffect(() => {
    if (isNotificationSuccess) {
      ...
      setNotificationAssessment([]);
      setDate("");
      setAssessor("");
      setAssessorComment("");
      setAssessmentScore("");
      setNotificationWorkHistory([]);
      setFromDate("");
      setToDate("");
      setPosition("");
      setContractType("");
      ...
      setNotificationCurrentEmployment({
        currentPosition: "",
        joinDate: "",
        contractType: "",
      ...
      navigate("/hr/notifications/"); //will navigate here after saving
    }
  }, [isNotificationSuccess, navigate]); //even if no success it will navigate and not show any warning if failed or success
  ...
  //setNotificationAssessment([]) no assessement for new notifications
  ...
  // to deal with notification work history entries:
  // Handler to update an entry field
  const handleWorkHistoryFieldChange = (index, field, value) => {
    const updatedEntries = [...notificationWorkHistory];
  ...
  const canSave =
    [
      ...
      basic,
      userRoles.length,
    ].every(Boolean) && !isNotificationLoading;
  //console.log(` ${userFirstName}, ${validUserLastName}, ${validUsername}, ${validPassword}, ${validUserDob},${userAllowedActions}    ${ validStreet},  ${validPrimaryPhone}, ${validEmail}, ${userRoles.length}, ${isParent}, ${notificationId}, ${userIsActive}` )
  const onSaveNotificationClicked = async (e) => {
    ...
    if (canSave) {
      ...
      //console.log(` 'first name' ${userFirstName}', fullfirstname,' ${userFullName.userFirstName}', house: '${house}', usercontact house' ${userContact.house},    ${userRoles.length},${isParent}, ${notificationId}` )
      ...
      if (isNotificationError) {
        console.log("error savingg", notificationError); //handle the error msg to be shown  in the logs??
      }
    }
  };
    ...
    <>
      ...
      <form className="form-container" onSubmit={onSaveNotificationClicked}>
        ...
        {/* <label className="form__label" htmlFor="isParent">
                ...
                />
                 <label className="form__label" htmlFor="notificationId">
                 User Is Notification: <span className="nowrap">[24 digits]</span></label>
                <input
                    className={`form__input `}
                    id="notificationId"
                    name="notificationId"
                    ...
                    onChange={onNotificationIdChanged}
                /> */}
        ...
        <label>
          <input
            ...
            onChange={onNotificationIsActiveChanged}
          />
