...
export const employeeDocumentsListsApiSlice = apiSlice.injectEndpoints({
  //define endpoints
  endpoints: (builder) => ({
    //a hook will be created automatically based on the end point :getemployeeDocumentsLists
    getEmployeeDocumentsLists: builder.query({
      query: () => "/settings/hrSet/employeeDocumentsLists/", //this route is as defined in the backend server.js to give all employeeDocumentsLists
      ...
      transformResponse: (responseData) => {
        const newLoadedEmployeeDocumentsLists = responseData.map(
        ...
        return employeeDocumentsListsAdapter.setAll(
          initialState,
          newLoadedEmployeeDocumentsLists
        );
      },
      providesTags: ["employeeDocumentsList"],
    }),
    getEmployeeDocumentsByYearById: builder.query({//different url called here
    ...
    getEmployeeDocumentsListById: builder.query({
      //will get the list for that year and append studtn document id if exisits
      query: (params) => {
        const queryString = new URLSearchParams(params).toString();
        return `/settings/hrSet/employeeDocumentsLists?${queryString}`;
      },
    ...
    updateEmployeeDocumentsList: builder.mutation({
      query: (initialEmployeeDocumentsListData) => ({
        url: "/settings/hrSet/employeeDocumentsLists/",
        method: "PATCH",
        body: {
...
export const selectEmployeeDocumentsListsResult =
  employeeDocumentsListsApiSlice.endpoints.getEmployeeDocumentsLists.select();
...
} = employeeDocumentsListsAdapter.getSelectors(
  (state) => selectEmployeeDocumentsListsData(state) ?? initialState
);
