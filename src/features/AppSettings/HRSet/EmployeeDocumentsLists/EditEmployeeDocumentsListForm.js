...
const [employeeDocumentsList, setEmployeeDocumentsList] = useState(listToEdit.documentsList || []);
...
const [updateEmployeeDocumentsList, {//an object that calls the status when we execute the newUserForm function
  isLoading,
  isSuccess,
  isError,
  error
...
const handleRemoveEntry = (index) => {
  setEmployeeDocumentsList(employeeDocumentsList.filter((_, i) => i !== index))
}



//to check if we can save before onsave, if every one is true, and also if we are not loading status
const canSave = [ documentsAcademicYear, ...employeeDocumentsList.map(entry => entry.documentTitle)].every(Boolean) && !isLoading


const onSaveEmployeeDocumentsListClicked = async (e) => {
  e.preventDefault()
  if (canSave) {
      await updateEmployeeDocumentsList({ id:listToEdit.id, documentsList: employeeDocumentsList, documentsAcademicYear })
      if (isError) {
