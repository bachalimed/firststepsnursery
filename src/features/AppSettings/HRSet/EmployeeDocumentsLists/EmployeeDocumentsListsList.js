...
import { useGetEmployeeDocumentsListsQuery, useDeleteEmployeeDocumentsListMutation} from "./employeeDocumentsListsApiSlice"
...
const EmployeeDocumentsListsList = () => {
  ...
  const [deleteEmployeeDocumentsList, {
...
let filteredEmployeeDocumentsLists
  ...
  if (isSuccess ) {
	...
	filteredEmployeeDocumentsLists = employeeDocumentsListsArray?.filter(item => {
...
{ 
  ...
  cell: row => (
    <div className="space-x-1">
      ...
      {canEdit?(<button  className="text-yellow-400" onClick={() => Navigate(`/settings/employeesSet/employeeDocumentsList/edit/${row.id}`)}  > 
      <FiEdit fontSize={20}/> 
      </button>):null}
      {canDelete?(<button className="text-red-500"  onClick={() => onDeleteEmployeeDocumentsListClicked(row.id)}>
...
<div className='flex-1 bg-white px-4 pt-3 pb-4 rounded-sm border border-gray-200'>
  ...
  <div className="flex justify-end items-center space-x-4">
    <button
      ...
      onClick={() => Navigate('/settings/employeesSet/newEmployeeDocumentsList')}
