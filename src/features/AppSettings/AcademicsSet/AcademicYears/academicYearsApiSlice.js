...
export const academicYearsApiSlice = apiSlice.injectEndpoints({
  //define endpoints
  endpoints: (builder) => ({
    //a hook will be created automatically based on the end point :getacademicYears
    getAcademicYears: builder.query({
      ...
      transformResponse: (responseData) => {
        ...
        //sort by newst year
        const sortedList = newAcademicYears.sort((a, b) =>
...
export const selectAcademicYearsResult =
  academicYearsApiSlice.endpoints.getAcademicYears.select();
...
} = academicYearsAdapter.getSelectors(
  (state) => selectAcademicYearsData(state) ?? initialState
);
