...
const User = ({ userId }) => {
  //userId is used in UsersList from the get query
  const user = useSelector((state) => selectUserById(state, userId));
  const navigate = useNavigate();

  if (user) {
    //if user exists we get all the data
    ...
    const userDob = new Date(user.userDob).toLocaleString("en-US", {
      day: "numeric",
      month: "numeric",
      year: "numeric",
    });
    ...
    return (
      <tr className="table__row user">
        ...
        {/**setting table cell status to active or not based on the user status */}
