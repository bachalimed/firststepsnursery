...
const {
  ...
  isError: isError,
  error: Error
} = useGetFamiliesByYearQuery({selectedYear:selectedYear ,endpointName: 'familiesList'}||{},{//this inside the brackets is using the listeners in store.js to update the data we use on multiple access devices
...
const onDeleteFamilyClicked = async (id) => {    
  ...
  cell: row => (
    <div className="space-x-1">
     <button className="text-blue-500" fontSize={20}  onClick={() => Navigate(`userDetails/${row._id}`)}  > 
      ...
      {canDelete?(<button className="text-red-500" onClick={() => onDeleteFamilyClicked(row._id)} >
