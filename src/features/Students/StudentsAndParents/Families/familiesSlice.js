...
const familiesSlice = createSlice({
    ...
    reducers: {
        setFamilies: (state, action) => {//get and sort the items
            familiesAdapter.setAll(state, action.payload)
        
        },   
        setSomeFamilies: (state, action) => {familiesAdapter.upsertMany(state, action.payload)
...
//export actions
export const { setFamilies, updateFamily,   setResult, setSomeFamilies } = familiesSlice.actions//to be used with dispatch in the components

export const currentParentsList = (state) => state.family
//export reducer
