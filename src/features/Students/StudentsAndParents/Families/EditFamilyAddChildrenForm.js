...
export default function EditFamilyAddChildrenForm() {
  ...
  useEffect(() => {
    if (isStudentListSuccess) {
      ...
      const selectedStu = () => {
        return allInitialStudents.filter(
  ...
  const handleChildChange = (id) => {
    ...
    const updatedStudents = allStudents.map(
      (student) =>
  ...
  const addChildDropdown = () => {
    ...
    firstAvailableStudent = allStudents.find((student) => !student.isSelected);
    ...
    if (firstAvailableStudent) {
      ...
      setChildren([
        ...children,
        { child: { ...firstAvailableStudent, isSelected: true } },
      ]);
  ...
  const removeChildDropdown = (childId) => {
    ...
    setSelectedStudents(
      updatedAllStudents.filter((student) => student.isSelected)
    );
    ...
    content = (
      <form className="max-w-lg mx-auto p-6 bg-white shadow-lg rounded-lg">
        ...
        {children.map((childObj) => {
          //this is the number of lines selected
              ...
              >
                ...
                {selectedChild ? (
                  <option value={selectedChild._id}>
                    {selectedChild?.studentName?.firstName}{" "}
                    {selectedChild?.studentName?.middleName}{" "}
                    {selectedChild?.studentName?.lastName}
                  </option>
                ) : (
